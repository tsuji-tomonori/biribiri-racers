...
export function tick(
  ...
  if (input.push) {
    r.charge = clamp(r.charge + dt / 0.85, 0, 1);
    r.boost = 0;
  }
  const target = input.push
    ? 52
    : t.speed * (input.throttle ?? 1) * (r.boost > 0 ? 1.65 : 1);
  ...
  const traction = 1 - Math.exp(-(input.push ? 10 : t.grip) * dt);

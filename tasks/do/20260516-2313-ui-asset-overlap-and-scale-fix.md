...
- `node --check app/web/app.js`: pass を確認
- ローカル静的配信（例: `python3 -m http.server 8027`）+ headless Chrome で
  - 1440px: menu / room / map / ready / result のスクショ取得
  - 375px: menu / map / room のスクショ取得
  - 参照画像と並べて二重テキスト・歪みがないか目視確認
- 主要導線手動操作（メニュー → ルーム作成 → コース選択 → ready → race → result）
...
- 受け入れ条件確認コメント: 未取得
- セルフレビューコメント: 未取得
- GitHub Apps 操作: 過去 403 のため `gh` フォールバック想定

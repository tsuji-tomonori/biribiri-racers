...
- [ ] コース02〜06の `boardAsset` 欠落が、実装またはテストで見逃されない状態になっている。
- [ ] visual regression の skip 状態について、解除できる場合は解除し、解除できない場合は理由と次工程が明示されている。
- [ ] `npm run build` が pass する。
- [ ] `npm run test:visual` が pass する。
- [ ] 未対応の完全一致作業を完了扱いにせず、作業レポートと PR に残している。

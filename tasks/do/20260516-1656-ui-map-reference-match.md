...
- [ ] メインのコースマップが参照画像に近い、太い灰色路面、カラフル電撃壁、START/GOALゲート、方向矢印を持つ。
- [ ] コースプレビュー、ミニマップ、リザルト内のコース表示が本体マップと視覚的に矛盾しない。
- [ ] 既存の一人プレイ操作、壁接触でスタートへ戻る挙動、ラップ完了、ブーストが維持されている。
- [ ] 320px相当のモバイル幅でも主要テキストと操作ボタンが重ならない。
- [ ] 未実装のオンライン要素を実データのように表示しない。

# アセット配置とコース表現の UI 仕上げ改善

- 状態: todo
- タスク種別: 修正
- 想定ブランチ: `codex/ui-asset-layout-polish`
- 対象: `app/web/`

## 背景

asset pack v2 を使った UI 反映後も、ユーザー確認で以下の違和感が出ている。
...
参照画像と asset pack v2 の意図に合わせ、画像アセットを「文字の背景」ではなく UI の主要ビジュアルとして扱う。コースはカード画像の単純な背景重ねではなく、コース部品を使った見える構成へ寄せ、主要画面で縮尺・重なり・可読性を安定させる。

## スコープ

- メニュー、マップ一覧、コース選択、コース詳細、準備画面の UI 仕上げを対象にする。
- `app/web/styles.css` のアセット背景・画像レイヤー・レスポンシブ配置を見直す。
- `app/web/app.js` の course metadata / asset mapping が必要なら最小限更新する。
- 既存のゲーム操作、コース選択、レース開始、未実装機能の honest state は維持する。
...
- [ ] 主要画面のボタン、コースカード、ショートカット、フッター操作で、意味のある文字がアセット画像の主要絵柄や文字入り部分に重なっていない。
- [ ] 視覚テキストを隠す場合でも、button の accessible name は維持され、キーボード操作と focus 表示が壊れていない。
- [ ] メニューの `track-preview` とコース詳細のミニマップが、少なくとも走路、スタート/ゴール、障害物または装飾 props を分離したレイヤー構成になっている。
- [ ] コース表現が course card 画像の単純背景ではなく、`course_parts` と theme assets を使った参照画像に近い見た目になっている。
- [ ] kart / badge / road segment / decorative prop の標準表示サイズまたは CSS custom property が定義され、同じ役割の asset が画面間で大きく縮尺ズレしない。
- [ ] 375px、768px、1440px の browser QA で、画像上文字の競合、横スクロール、主要 UI の重なり、テキスト破綻がない。
- [ ] 既存のコース選択、ルーム作成、レース開始、リザルト表示の操作が壊れていない。
- [ ] 未実装のオンライン機能、ランキング、履歴、フレンド機能を実データのように表示していない。
- [ ] 選定した lint / syntax / browser QA が pass し、未実施項目がある場合は理由を PR 本文または作業レポートに記録している。

## 検証計画

- `git diff --check`
- `node --check app/web/app.js`
- `python3 -m http.server <port>` で静的配信
- headless Chrome または Playwright で 375px / 768px / 1440px の menu / room / map / ready を確認
- 必要に応じて screenshot を `/tmp/` に保存し、重なり・縮尺・参照コースとの差分を作業レポートに記録

...
- PR: https://github.com/tsuji-tomonori/biribiri-racers/pull/11
- Commit: `538a167`
- 作業レポート: `reports/working/20260517-1145-asset-pack-v2-visual-refresh.md`
- 実行した検証:
  - `npm install`: pass
  - `npm run build`: pass
  - `git diff --check`: pass
  - headless Chrome screenshot: `/`, `/?screen=room`, `/?screen=join`, `/?screen=map`, `/?screen=ready`, `/?screen=result`
- 未検証:
  - 手動でレースを完走する E2E 操作。`app/web/src/hooks` と `app/web/src/game` の操作・当たり判定ロジックは変更していない。

...
- 全コースの旧出口ポリゴンを閉鎖。シティの下部外壁、キャンディの右下カーブ、ガーデンの下部外壁を新しい画像の道路端に合わせた。ガーデンのCPU経路を道路中央へ寄せた。
...
PR #19 CIは68 unit・27 E2E成功（6件は意図的スキップ）。PC/375px/768pxの全コース画面確認済み。Pagesの新revision反映後、既存ブラウザで旧画像キャッシュの表示を発見。画像URLにビルドSHAを付与して再公開し、同じブラウザで新画像への更新を確認した。
...
## 公開確認完了
PR #19・#20をmainへマージ。公開版 `2e7057473ea479e763931c8b84f6254065c979be` のActions run `33955738928` はverify/deploy/smoke全成功。68 unit・27 E2E成功、6件は端末重複の意図的スキップ。同じブラウザで新revision・新画像URL・旧GOALなしを確認。ガーデンCPUふつうで、さくらが36.64秒・接触0回・ダッシュ7回で完走した。ゲーム資産・コードの未対応事項なし。

公開URL: https://tsuji-tomonori.github.io/biribiri-racers/
